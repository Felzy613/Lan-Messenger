...
| Selected row | Fluent accent pill at the leading edge | A `glass-thick` lozenge with the rim at `radius-row`, from `GlassListItemStyle`'s own `ListViewItem` template. Recolouring Fluent's presenter through `ListViewItemBackgroundSelected*` removes the accent pill but leaves the highlight square: the presenter ignores both `CornerRadius` and `ListViewItemCornerRadius`. |
...
| Content dialogs | Default | `GlassDialogStyle`, a `ContentDialog` template of our own: thick acrylic at 26px with the rim, no separator or second background above the buttons, buttons at the trailing edge at their own width, and no restyle of the default button (Fluent's makes it a square accent button). `GlassDialog.Apply` sets it and the button roles; with `scrollingBody: true` it sets `GlassScrollingDialogStyle`, the same sheet on an opaque ground, for a sheet whose whole body scrolls (see Settings). |
| Settings | Cards with radius 6 | Grouped `glass-thick` panels with radius 20 and `divider` between rows. Each row is a label (and hint) with one trailing control: a `GlassRowToggleStyle` switch with no header or On/Off caption, a folder path with a plain "Change…", or a `GlassTextFieldStyle` field. The dialog carries the title; the page has none. The body scrolls under soft edges (`SheetScrollEdges`): rows dissolve into the sheet over `space-24` under the title and above Done, only while there is content beyond that edge, never cut by the viewport. The fade must end in exactly the sheet's colour, which acrylic does not have, so this sheet is `GlassScrollingDialogStyle`: an opaque `glass-regular-fallback` ground, which also keeps the `glass-thick` groups a step lighter than the sheet (on `glass-thick-fallback` they would melt into it). |
| Text fields | Fluent `TextBox`, 4px, accent underline on focus | `GlassTextFieldStyle`: `inset-fill`, `glass-edge` outline, `radius-card`, the 2px `focus-ring` drawn over the outline on focus. `GlassSearchFieldStyle` is the same as a capsule with room for a magnifier. |
| Title bar | System title bar (takes the user's accent colour) | Drawn by the app on the Mica: a 32px row with the icon and title in `caption` `ink-secondary`, and the caption buttons transparent with `glass-hover` and `glass-pressed`. |
