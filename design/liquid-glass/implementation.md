...
| Content dialogs | Default | `GlassDialogStyle`, a `ContentDialog` template of our own: thick acrylic at 26px with the rim, no separator or second background above the buttons, buttons at the trailing edge at their own width, and no restyle of the default button (Fluent's makes it a square accent button). `GlassDialog.Apply` sets it and the button roles. |
| Settings | Cards with radius 6 | Grouped `glass-thick` panels with radius 20 and `divider` between rows. Each row is a label (and hint) with one trailing control: a `GlassRowToggleStyle` switch with no header or On/Off caption, a folder path with a plain "Change…", or a `GlassTextFieldStyle` field. The dialog carries the title; the page has none. |
