...
### G1. Token generator
- [x] Add `scripts/design/gen_tokens.py` (Python 3, standard library only). It reads `design/liquid-glass/tokens.json` and writes three files, each starting with `// GENERATED by scripts/design/gen_tokens.py from design/liquid-glass/tokens.json. Do not edit.` (use `<!-- … -->` in XAML):
...
- [x] **Colour parsing.** `#rrggbb` becomes alpha FF. `rgba(r, g, b, a)` becomes alpha `round(a × 255)`. `{name}` resolves to that token's value *for the same theme*; follow chains, and error on a cycle or a missing name. A plain string value applies to both themes.
- [x] **Naming.** Token `accent-ink-out` becomes Swift `accentInkOut` and C#/XAML `AccentInkOut`. Radius, spacing and size tokens drop their family prefix inside a nested namespace (`radius-bubble` becomes `GlassTokens.Radius.bubble` / `GlassTokens.Radius.Bubble`).
- [x] **Swift output:**
  ```swift
  import SwiftUI
  import AppKit
  ...
  ```
- [x] **C# output.** A `public static class GlassTokens` in `LanMessenger.UI`, holding `public static readonly Windows.UI.Color BrandLight, BrandDark, …` for every colour token, `public static Color Pick(Color light, Color dark) => Theme.IsDark ? dark : light;`, nested `Radius`, `Space`, `Size` and `Acrylic` classes of `double` constants, and one `Color` per theme for each fallback.
- [x] **XAML output.** A `ResourceDictionary` with `ThemeDictionaries` for `Light`, `Dark` and `HighContrast`:
  ...
  - *As built:* the radius doubles are `TokenRadius<Name>Value`, since `TokenRadius<Name>` is the `CornerRadius`. The type ramp is generated too, as `TextBlock` styles `TokenType<Name>Style` (size, line height, weight; `TokenMonoFontFamily` for `code`). Swift also gets `GlassTokens.Raw` (every colour as a `GlassColorPair` of ARGB values, for tests and for code holding an explicit `ColorScheme`), `Typography`, `Shadow` and `Acrylic`; C# gets `GlassTokens.All` for tests.
- [x] **HighContrast** maps by role, without acrylic or gradients:
  ...
  Two corrections made while implementing: `*-wash` tokens map to `SystemColorWindowColor`, not with their `accent-*` / `warning*` / `danger*` family to Highlight, because a wash is the fill *behind* accent content and Highlight on Highlight is unreadable. `presence-ring` (missing from the table) maps to `SystemColorWindowColor`. The generator refuses a token with no High Contrast role, so every new token needs a deliberate one.

- [x] `--check` mode regenerates into memory and exits 1, with a diff, if any output differs from disk.
- [x] Add a step to `.github/workflows/pr-checks.yml` in **both** jobs, before tests: `python3 scripts/design/gen_tokens.py --check` (`python` on the Windows runner).

**Done when:** generation is deterministic (running it twice gives no diff), `--check` passes, and the Swift and C# files compile.

### G2. Token tests
- [x] `src/macos/LanMessengerTests/GlassTokensTests.swift` and `src/windows-native/LanMessenger.Tests/GlassTokensTests.cs`, with the same cases on both sides:
  ...
  Also asserted, beyond this table: `ink`, `ink-secondary` and `meta-out` on the opaque `-fallback` surfaces at 4.5 (transparency off), `accent-ink` on `accent-wash` over bubble-in and both glasses, `warning-ink` on `warning-wash` over both glasses, and that `AvatarPalette` equals the `avatar-*` tokens in order.
...
*Follow-up (2026-09-24), after the 2.1.0 build was seen on the Dell:* the per-dialog `AccentButton*` overrides only recoloured Fluent's default-button restyle, which stayed square (a green, square "Cancel" beside a round "Add Contact" on New Message), and the sheet kept Fluent's separator line and half-width lone button. `GlassDialogStyle` is now a full `ContentDialog` template and the overrides are gone. Text fields got a template of their own (`GlassTextFieldStyle`), Settings was rebuilt as label-and-trailing-control rows per `SettingsGroup` (no duplicate title, no On/Off captions, folders as paths with "Change…"), the New Message, Contacts, Find Contacts and Archived pages lost their remaining Fluent brushes and default buttons, list rows got their own template (`GlassListItemStyle`) because Fluent's highlight stayed square at any radius, the conversation row's options button moved under the time and became the row's right-click menu too, and the main window draws its own title bar instead of the system's accent-coloured one. Testing the Archived page (archive, Unarchive, Open, Back) found the "select a conversation" placeholder showing through its glass, so `MainWindow` now collapses the placeholder whenever a page is showing; a conversation with no messages no longer keeps an empty preview line under its name; and list rows have accessible names (they were read as the view-model type).

*Follow-up (2026-09-25), after 2.2.3 was seen on the Dell:* scrolled, the Settings sheet looked broken. Its viewport cut rows with a hard edge: "Reset" and "Change…" sliced in half right under the title, the Cloud relay panel sheared flat above Done. At the startup window size the sheet shows about half its content, so some row is always cut. The body now scrolls under soft edges (`UI/SheetScrollEdges.cs`): rows dissolve into the sheet over `space-24` under the title and above Done, only while there is content beyond that edge, and focus is brought into view clear of them. Because WinUI has no opacity mask, the edges are gradients in the sheet's colour and have to match it exactly, so the Settings sheet moved off acrylic onto an opaque `glass-regular-fallback` ground (`GlassScrollingDialogStyle`), and the edge colour is read from the ground and sheen the sheet actually paints. Measured from the Dell screenshot, the acrylic sheet ran from 243 to 249 top to bottom, and on a `glass-thick-fallback` ground the groups lost their tone and read as outlines (checked in an HTML mock of the sheet at 640×632 with the token values). Not yet seen on the Dell.
